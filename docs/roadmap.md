...
- `setLabel` / `get-label` — **removed** (ce1a345, 0.9.0). No reader ever existed; `api.setLabel` and the manager's `entry-labels` atom are gone, and the capability lint map no longer lists it.
