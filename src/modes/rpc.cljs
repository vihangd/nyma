...
(defn ^:async start [agent]
  (let [rl (readline/createInterface
    ...
    (.on rl "line"
      (fn ^:async [line]
        (try
          (let [cmd (js->clj (js/JSON.parse line) :keywordize-keys true)]
            (case (:type cmd)
              "prompt"       (js-await (run agent (:message cmd)))
              "abort"        nil
              "get_commands" (println (js/JSON.stringify
                                       (clj->js @(:commands agent))))
              "get_settings" (println (js/JSON.stringify
                                       (clj->js (:config agent))))
