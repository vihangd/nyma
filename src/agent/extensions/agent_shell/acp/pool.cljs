...
(defn- create-connection
  ...
  (let [command      (:command agent-def)
        ...
        ;; Create a real emit function that notifies via api.ui
        emit-fn  (fn [event data]
                   (try
                     ;; Emit on the global event bus via api.on's underlying bus
                     ;; Since extensions can't emit directly, use ui.notify for key events
                     (when (and (.-ui api) (.-available (.-ui api)))
                       (case event
                         "acp_message"  nil  ;; handled by prompt accumulation
                         "acp_thought"  nil  ;; rendered via thought-callback in input-router
