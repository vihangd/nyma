...
(defn ^:async handle-message
  ...
  (let [{:keys [create-session-fn agent-opts streaming-policy
        ...
        on-agent-end  (fn [data]
                        ;; Flush the streaming buffer, then signal done. Once
                        ;; per run: agent_end has a single producer now (the
                        ;; loop's explicit emit), so a second :done here would
                        ;; be a loop bug, and gateway_loop.test pins it.
