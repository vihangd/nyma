...
(def ^:private test-home (atom nil))
...
(beforeEach
 (fn []
   ...
   (reset! test-home (make-temp-dir))))

(afterEach
 (fn []
   (cleanup-dir @test-dir)
   (cleanup-dir @test-home)
   (reset! test-dir  nil)
   (reset! test-home nil)))
...
(describe "mcp-discovery:scan-mcp-servers" (fn []
                                             (it "returns empty when no config files exist"
                                                 (fn []
                                                   (let [result (mcp-discovery/scan-mcp-servers @test-dir @test-home)]
      ...
      ;; Set a test env var
                                                   ...
                                                   (let [result  (mcp-discovery/scan-mcp-servers @test-dir @test-home)
                                                         ...
                                                         api-key (some #(when (= (:name %) "API_KEY") %) env)]
        ;; env is now [{name, value}] array format (ACP schema-valid)
                                                     (-> (expect (= 1 (count env))) (.toBe true))
                                                     (-> (expect (:value api-key)) (.toBe "secret123")))
      ;; Cleanup env var
...
(describe "mcp-discovery:activate" (fn []
                                     ...
                                     (it "/mcp list with no servers shows helpful message"
                                         (fn []
                                           (let [api     (make-mock-api)
                                                 _       (mcp-discovery/activate api)
                                                 ;; activate rescans process.cwd + real home; force empty
                                                 _       (reset! shared/mcp-servers [])
