...
(describe "permissions" (fn []
                          ...
                          (it "parse-capabilities nil returns the default set, not :all"
                              (fn []
                                (let [caps (parse-capabilities nil)]
                                  (-> (expect (contains? caps :all)) (.toBe false))
                                  (-> (expect (contains? caps :tools)) (.toBe true))
                                  (-> (expect (contains? caps :exec)) (.toBe false))
                                  (-> (expect (contains? caps :spawn)) (.toBe false))
                                  (-> (expect (contains? caps :tools-override)) (.toBe false))
                                  (-> (expect (contains? caps :middleware)) (.toBe false)))))
                          ...
                          (it "round-trip: nil parse grants the safe set and withholds exec"
                              (fn []
                                (let [caps (parse-capabilities nil)]
                                  (-> (expect (check caps :tools)) (.toBe true))
                                  (-> (expect (check caps :ui)) (.toBe true))
                                  (-> (expect (check caps :middleware)) (.toBe false))
                                  (-> (expect (check caps :exec)) (.toBe false)))))))
